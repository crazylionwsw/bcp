angular.module('app')
    .directive('customerimageBlock', function() {
        return {
            restrict: 'E',
            scope: {bill:'=', canAudit:'=?'},
            templateUrl: 'tpl/view/customerimage/block.html',
            controller: function($scope, $element, $filter, $timeout, FileUploader, customerimageService, billtypeService, $modal, $rootScope, localStorageService,toaster){
                $scope.type = {};
                $scope.fileIds = [];
                $scope.customerImage = {};
                $scope.signModal = {};
                ...
                $scope.$on('CreditReportGenerated',function (e, cr) {
                    angular.forEach($scope.imagetypes, function(item, k){
                        if (item.code == 'B013') {
                            if (cr.customerId == $scope.bill.customerId) {
                                if ($scope.imagetypes[k].customerImage == null) $scope.imagetypes[k].customerImage = {};
                                $scope.imagetypes[k].customerImage.fileIds = [];
                                $scope.imagetypes[k].customerImage.fileIds.push(cr.padFileId);
                                $scope.imagetypes[k].customerImage.coverFileId = cr.padFileId;
                            }
                        }
                        if (item.code == 'B014') {
                            if (cr.customerId == $scope.bill.mateCustomerId) {
                                if ($scope.imagetypes[k].customerImage == null) $scope.imagetypes[k].customerImage = {};
                                $scope.imagetypes[k].customerImage.fileIds = [];
                                $scope.imagetypes[k].customerImage.fileIds.push(cr.padFileId);
                                $scope.imagetypes[k].customerImage.coverFileId = cr.padFileId;
                ...
                //  接受广播，更新$scope中的imageTypeIds
                //生成文档后上传至文档资料框  更新列表
                $scope.$on('UpdateContract',function (e, document) {
                    angular.forEach($scope.imagetypes, function(imagetype, imagetypeIndex){
                        angular.forEach(document.imageTypeCodes,function (item, index) {
                            if (imagetype.code == item){
                                if ($scope.imagetypes[imagetypeIndex].customerImage == null) {
                                    $scope.imagetypes[imagetypeIndex].customerImage = {};
                                    $scope.imagetypes[imagetypeIndex].customerImage.fileIds = [];
                                }
                                $scope.imagetypes[imagetypeIndex].customerImage.fileIds.push(document.customerContract.fileId);
                                $scope.imagetypes[imagetypeIndex].customerImage.coverFileId = document.customerContract.fileId;
                            }
                        });
                    })
                });
                        ...
                        if ($scope.bill.billType) {
                            ...
                            customerimageService.getParamImageCodesByBillTypeCodeAndBusinessTypeCode($scope.bill.businessTypeCode,$scope.bill.billTypeCode,function (imageTypeCodesList) {
                                ...
                                if (imageTypeCodesList){
                                    ...
                                    //通过customerId, customerTransactionId和单据所需要的档案类型获取客户档案列表
                                    customerimageService.getByCustomerIdAndCustomerTransactionIdAndImageTypeCodes($scope.bill.customerId,$scope.bill.customerTransactionId,imageTypeCodesList,function(data){
                                        var images = data;
                                        if (images) {
                                            //判断是否显示删除按钮
