 ...
 */
angular.module('app')
    .directive('declarationContractBlock', function(documentService,orderService, $filter,declarationService) {
        return {
            restrict: 'E',
            scope: {bill: '='},
            ...
            link: function ($scope, element, attrs) {
                ...
                $scope.$watch('bill', function (bill) {
                    if ($scope.bill) {
                            //  获取 单据 所必须的   需要打印的文档类型
                            var documentIds = bill.documentIds;
                            //      根据  客户ID 、交易ID以及该单据必须的合同IDS,获取客户的合同信息
                            documentService.getByCustomerIdAndCustomerTransactionIdAndDocumentIds($scope.bill.customerId,$scope.bill.customerTransactionId,documentIds,function (data) {
                                ...
                                if (customerContracts){
                                    ...
                                    documentService.getTypesByIds(documentIds, function (data) {
            ...
            controller: function($scope, $element, $filter, $cookieStore, $location, customerService,documentService,sysparamService,orderService,$state,$modal,$rootScope) {
                ...
                $scope.view = function(document) {
                    ...
                    $scope.save(document, function(fileId){
                        if (document.fileType == "docx" || document.fileType == "doc"){
