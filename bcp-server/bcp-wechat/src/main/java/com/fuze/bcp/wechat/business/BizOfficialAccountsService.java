 ...
 */
@Service
public class BizOfficialAccountsService implements IOfficialAccountsService {

    Logger logger = LoggerFactory.getLogger(BizOfficialAccountsService.class);
    ...
    @Value("${fuze.bcp.wechat.public.appid}")
    private String appid;

    @Value("${fuze.bcp.wechat.public.secret}")
    ...
    public ResultBean<JSONObject> baseAccessToken(Boolean flag) throws IOException {
        ...
        try {
            ...
            if (!flag) {
                tokenStr = jedis.hget("base_access_token", secret);
            }
            if (tokenStr == null) {
            ...
            if (jsonObject != null) {
                if (!jsonObject.has("errcode") || jsonObject.getInt("errcode") == 0) {
                    ...
                    jedis.expire("base_access_token", jsonObject.getInt("expires_in"));
                    return ResultBean.getSucceed().setD(jsonObject);
                } else {
    ...
    public ResultBean<JSONObject> baseJsapiTicket(Boolean flag) throws IOException {
        ...
        try {
            ...
            if (!flag) {
                ticketStr = jedis.hget("ticket_" + appid, secret);
            }
            if (ticketStr == null) {
            ...
            if (!jsonObject.isNullObject()) {
                logger.info("判断errcode是否与0相等" + (jsonObject.getInt("errcode") == 0));
                if (!jsonObject.has("errcode") || jsonObject.getInt("errcode") == 0) {
                    logger.info("向redis中保存ticket:" + ticketStr);
                    jedis.hset("ticket_" + appid, secret, ticketStr);
                    jedis.expire("ticket_" + appid, jsonObject.getInt("expires_in"));
                    return ResultBean.getSucceed().setD(jsonObject);
                } else {
                    logger.info("错误" + jsonObject.getInt("errcode") + ":" + jsonObject.getString("errmsg"));
